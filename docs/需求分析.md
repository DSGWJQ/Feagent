...
### 正确的开发顺序
1. **需求分析**（已完成）→ 理解业务需求
2. **Domain 层设计**（TDD 驱动）→ 从业务出发，设计实体、值对象
   - 先写测试（Red）
   - 再写实现（Green）
   - 最后重构（Refactor）
3. **Ports 定义** → 定义 Repository、外部服务接口
4. **Infrastructure 层** → 实现 ORM 模型、Repository
5. **数据库迁移** → 使用 Alembic 生成迁移脚本
6. **Application 层** → 用例编排、事务边界
7. **API 层** → FastAPI 路由、DTO

### 错误的开发顺序（禁止）
❌ 数据库设计 → Domain 层 → ORM 模型 → 数据库迁移
- **问题**：违反 DDD 原则，Domain 层依赖数据库
- **后果**：业务逻辑与技术实现耦合，难以测试

### 开发节奏要求
- **一次只做一个小步骤**（例如：只写一个测试 + 实现）
- **每完成一个小步骤**，停下来等待确认
- **禁止一次性创建大量代码**（超过 2 个文件视为违规）
- **每一步都要说明**：现在做什么、为什么、下一步怎么做
- 后端规范（摘要）：
...
详细规范见 [docs/DEVELOPMENT_GUIDE.md](./DEVELOPMENT_GUIDE.md)。
