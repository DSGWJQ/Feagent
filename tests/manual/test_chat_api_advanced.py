...
# 测试场景 1: 添加多个节点
test_chat(
    "添加一个HTTP节点用于获取用户数据，然后添加一个LLM节点用于分析数据", "测试 1: 添加多个节点"
)

# 测试场景 2: 修改节点配置
test_chat("把HTTP节点的URL改成 https://api.github.com/users/octocat", "测试 2: 修改节点配置")
...
# 测试场景 4: 重新组织工作流
test_chat(
    "删除所有节点，重新创建一个简单的工作流：开始 -> HTTP请求 -> 结束", "测试 4: 重新组织工作流"
)

# 测试场景 5: 添加Python节点
test_chat("在HTTP请求后添加一个Python节点，用于处理返回的JSON数据", "测试 5: 添加Python节点")
