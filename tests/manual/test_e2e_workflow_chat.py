...
def test_e2e_workflow_chat():
    ...
    chat_response = requests.post(
        ...
        json={"message": "在开始和结束之间添加一个HTTP节点，用于获取天气数据"},
    )
    ...
    chat_response2 = requests.post(
        f"{API_BASE_URL}/api/workflows/{workflow_id}/chat",
        json={"message": "在HTTP节点后添加一个LLM节点和一个数据库节点"},
    )
    ...
    chat_response3 = requests.post(
        f"{API_BASE_URL}/api/workflows/{workflow_id}/chat", json={"message": "删除数据库节点"}
    )
