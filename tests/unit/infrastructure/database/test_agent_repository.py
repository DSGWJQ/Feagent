...
class TestAgentRepositorySave:
    ...
    @pytest.mark.asyncio
    async def test_save_new_agent_should_persist_to_database(self, agent_repository, sample_agent):
        ...
        # Assert：验证保存成功
        found_agent = await agent_repository.get_by_id(sample_agent.id)
    ...
    @pytest.mark.asyncio
    async def test_save_existing_agent_should_update_database(self, agent_repository, sample_agent):
...
class TestAgentRepositoryGetById:
    ...
    @pytest.mark.asyncio
    async def test_get_by_id_existing_agent_should_return_agent(
    ...
    @pytest.mark.asyncio
    async def test_get_by_id_non_existing_agent_should_raise_not_found_error(
        ...
        with pytest.raises(NotFoundError) as exc_info:
            await agent_repository.get_by_id(non_existing_id)
...
class TestAgentRepositoryFindById:
    ...
    @pytest.mark.asyncio
    async def test_find_by_id_existing_agent_should_return_agent(
        self, agent_repository, sample_agent
    ):
        ...
        # Act
        found_agent = await agent_repository.find_by_id(sample_agent.id)
    ...
    @pytest.mark.asyncio
    async def test_find_by_id_non_existing_agent_should_return_none(self, agent_repository):
        ...
        """
        # Act
        found_agent = await agent_repository.find_by_id("non-existing-id")
...
class TestAgentRepositoryFindAll:
    ...
    @pytest.mark.asyncio
    async def test_find_all_with_multiple_agents_should_return_all_agents(self, agent_repository):
        ...
        await agent_repository.save(agent3)
    ...
    @pytest.mark.asyncio
    async def test_find_all_with_no_agents_should_return_empty_list(self, agent_repository):
...
class TestAgentRepositoryExists:
    ...
    @pytest.mark.asyncio
    async def test_exists_with_existing_agent_should_return_true(
        self, agent_repository, sample_agent
    ):
        ...
        # Act：检查 Agent 是否存在
        exists = await agent_repository.exists(sample_agent.id)

        # Assert：验证返回 True
        assert exists is True, "存在的 Agent 应该返回 True"

    @pytest.mark.asyncio
    async def test_exists_with_non_existing_agent_should_return_false(self, agent_repository):
        ...
        """
        # Act：检查不存在的 Agent
        exists = await agent_repository.exists("non-existing-id")
...
class TestAgentRepositoryDelete:
    ...
    @pytest.mark.asyncio
    async def test_delete_existing_agent_should_remove_from_database(
        self, agent_repository, sample_agent
    ):
        ...
        # Arrange：保存 Agent
        await agent_repository.save(sample_agent)
        assert await agent_repository.exists(sample_agent.id) is True

        # Act：删除 Agent
        await agent_repository.delete(sample_agent.id)

        # Assert：验证删除成功
        assert await agent_repository.exists(sample_agent.id) is False, "删除后应该不存在"
        found_agent = await agent_repository.find_by_id(sample_agent.id)
        assert found_agent is None, "删除后应该查询不到"

    @pytest.mark.asyncio
    async def test_delete_non_existing_agent_should_not_raise_error(self, agent_repository):
        ...
        await agent_repository.delete("non-existing-id")
