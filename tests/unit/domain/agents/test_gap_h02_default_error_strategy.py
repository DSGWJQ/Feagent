...
class TestWorkflowPlanDefaultErrorStrategy:
    ...
    def test_default_error_strategy_is_optional(self):
        ...
        assert hasattr(plan, "default_error_strategy"), \
            "WorkflowPlan 应该有 default_error_strategy 字段"
        # 明确验证默认值
        assert plan.default_error_strategy is None or plan.default_error_strategy == {}, \
    ...
    def test_node_without_strategy_and_no_default_returns_none(self):
        ...
        assert effective_strategy is None, \
            "无默认策略且节点未配置时应返回 None"
...
class TestErrorStrategyInheritance:
    ...
    def test_node_inherits_default_strategy(self):
        ...
        assert effective_strategy["retry"]["max_attempts"] == 3, \
            "应该继承默认策略的重试次数"
    ...
    def test_node_strategy_overrides_default(self):
        ...
        # 节点策略应该覆盖默认策略
        assert effective_strategy["retry"]["max_attempts"] == 5, \
            "节点策略应该覆盖默认策略"
        assert effective_strategy["on_failure"] == "abort", \
            "节点的 on_failure 应该覆盖默认"
    ...
    def test_partial_node_strategy_merges_with_default(self):
        ...
        assert effective_strategy["retry"]["max_attempts"] == 3, \
            "retry 应该从默认策略继承"
        assert effective_strategy["on_failure"] == "skip", \
            "on_failure 应该被节点策略覆盖"
...
class TestErrorStrategySerialization:
    ...
    def test_to_dict_includes_default_error_strategy(self):
        ...
        assert "default_error_strategy" in data, \
            "序列化结果应该包含 default_error_strategy"
...
class TestErrorStrategyValidation:
    ...
    def test_invalid_on_failure_value(self):
        ...
        assert any("on_failure" in err for err in errors), \
            "应该报告无效的 on_failure 值"
    ...
    def test_valid_on_failure_values(self):
        ...
        for value in valid_values:
            ...
            assert len(on_failure_errors) == 0, \
                f"'{value}' 应该是有效的 on_failure 值"
    ...
    def test_retry_max_attempts_range(self):
        ...
        errors = plan.validate()
        assert any("max_attempts" in err for err in errors), \
            "负数重试次数应该无效"
