/**
 * WorkflowCreatePage - 显式创建入口（/workflows/new）
 *
 * Contract:
 * - 通过 /api/workflows/chat-create/stream 创建 workflow
 * - 必须携带显式创建 Header（由 API client 层统一注入）
 *
 * SOLID:
 * - SRP: 创建页只负责“创建 + 跳转”，编辑由 WorkflowEditorPage 负责
 */
...
export const WorkflowCreatePage: React.FC = () => {
  ...
  const createNewWorkflow = useCallback(async () => {
    ...
    try {
      ...
      // 成功拿到 workflow_id 后会 navigate，组件可能被卸载；这里不强制 setIsCreating(false)
    }
      ...
      <Card style={{ maxWidth: 720, width: '100%' }}>
        <Space direction="vertical" size="middle" style={{ width: '100%' }}>
          ...
          <Space>
            ...
            <Button onClick={() => navigate('/', { replace: true })} disabled={isCreating}>
              返回对话
            </Button>
