"""AgentRepository Port - 定义 Agent 实体的持久化接口

为什么需要 AgentRepository Port？
...
class AgentRepository(Protocol):
    ...
    def delete(self, agent_id: str) -> None:
        ...
        实现要求：
        - 必须是原子操作（事务内完成）
        - 幂等：多次删除同一个 Agent 不报错

        注意：
        - 删除 Agent 时，关联的 Run 怎么办？
        - 选项 1：级联删除（删除 Agent 时删除所有 Run）
        - 选项 2：禁止删除（如果有 Run，抛出异常）
        - 选项 3：软删除（标记 Agent 为 archived）
        - 具体策略由业务决定，实现层负责执行
        """
        ...
